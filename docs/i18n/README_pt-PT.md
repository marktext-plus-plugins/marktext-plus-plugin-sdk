...
Uma extensão corre em máquinas onde só está o editor — nem SDK do Dart, nem Node, nem Python. Este único facto decide quase tudo o que se segue.
...
Um script, um manifesto e três ficheiros de documentação. **Nem sinal de Dart.**
...
- um objecto JSON por linha, em stdin e stdout;
- as respostas devolvem o `id` numérico;
- terminar quando stdin chegar ao fim;
- terminar se `MARKTEXT_PLUS_PLUGIN_TOKEN` não estiver no ambiente, para que um executável aberto com duplo clique diga o que é em vez de ficar à espera.

É este o contrato todo. Aqui está em Python, sem nada tirado deste repositório — responde ao editor e recusa-se a correr se ninguém o arrancou:
...
`lua` e `js` são deliberadamente **a mesma extensão escrita duas vezes** — mesmo manifesto, mesmas permissões, mesmo comportamento — para que se possam ler lado a lado. **Copie uma.** Uma extensão declara um `runtime` e um ponto de entrada; um directório com as três são três extensões a vestir um só manifesto.
...
`require` carrega um dos **seus próprios** ficheiros — o módulo de API acima é carregado exactamente assim, e o mesmo vale para tudo o que puser ao lado:
...
Carregado uma só vez, por mais vezes que seja pedido. O nome é um nome, não um caminho: resolve-se dentro do directório da sua extensão e em mais lado nenhum. Partir uma extensão grande — ou distribuir com ela uma biblioteca escrita por outra pessoa — não lhe custa, portanto, qualquer acesso adicional ao resto do disco. Um nome com um separador, com `..` ou com um ponto inicial é recusado antes de se ler seja o que for, e o ficheiro resolvido é depois verificado como estando dentro do seu directório — é isso que apanha uma ligação simbólica que aponta para fora.
...
Uma extensão `process` não usa `entrypoint`. Nomeia os seus executáveis **por sistema operativo**, com a arquitectura por baixo e só onde importa:
...
O sistema é a parte a que tem sempre de responder — uma compilação para Windows é um ficheiro diferente de uma para Linux. A arquitectura muitas vezes não, por isso é opcional:

- **Um caminho para o sistema inteiro.** O `macos` acima é um binário universal: um ficheiro que contém ambas as arquitecturas, como as compilações de macOS costumam ser distribuídas. Escrever o mesmo caminho duas vezes para o dizer seria pior.
- **Uma tabela de arquitecturas.** O `windows` acima distribui uma compilação separada para cada uma e não suporta mais nada.
- **Um `default` partilhado mais uma especialização.** O `linux` acima corre `bin/linux/plugin` em toda a parte excepto em arm64, que tem a sua. Ganha a especializada.

Os sistemas são `windows`, `macos` e `linux`; as arquitecturas, `x64` e `arm64`. **Qualquer outro nome é recusado ao instalar em vez de ser saltado** — de outro modo um `windwos` mal escrito transformar-se-ia, no momento do clique, em «esta extensão não suporta a sua plataforma», sem nada com que o explicar.
...
Ambos os motores de script são síncronos — o interpretador de Lua não tem co-rotinas, e o motor de JS não tem ciclo de eventos próprio. Tudo o que leva tempo (perguntar a quem lê, chamar um modelo) bloquearia o editor. Por isso um script nunca espera: **devolve uma acção** que descreve o que quer que seja feito, o editor fá-lo, e volta a chamar o script com a resposta.
...
### Um painel pode ser questionado de novo

Um comando iniciado a partir de um painel pode ser executado uma segunda vez:
quem lê pede que a resposta seja alterada em vez de descrever tudo outra vez. O
editor di-lo nos campos que o seu script já conhece:

- `ctx.selection` é **a sua própria última resposta**, porque um pedido
  seguinte diz respeito ao rascunho à frente dos olhos, não ao documento.
- `ctx.answer` é o que foi pedido desta vez — o mesmo campo onde chega a
  resposta ao seu `ask`.

Um comando que lê a selecção como «a parte a trabalhar» não precisa de nada de
novo. Um que ignora `ctx.selection` reescreverá o documento do início de cada
vez, o que não é o que foi pedido.

A caixa só é oferecida se o comando tiver perguntado algo no início, pois é aí
que chega o pedido seguinte; um comando que nunca pergunta corre uma vez. O que
`apply` substitui continua a ser o que a primeira resposta indicou: um segundo
rascunho mais curto substitui na mesma o parágrafo de que o primeiro nasceu, e
não o rascunho de que provém, que não está no documento.

### As acções

| Devolve | O que o editor faz | Depois |
|---|---|---|
| `{ ask = "…", default = "…", choices = {…} }` | pergunta a quem lê; as `choices` aparecem como fichas para premir, e o que for escrito em vez delas é tomado tal como está | volta a chamar `on_command` com `ctx.answer` preenchido |
...
**A instrução é sua.** O editor guarda as credenciais e faz o pedido; não reescreve a sua instrução, e o seu script nunca vê a chave da API. Ao modelo é apresentada exactamente a cadeia que devolveu em `ai`.

### A que é que um script consegue chegar

Só a isto. Sem `os`, sem `package`, sem `dofile`, sem `loadfile`, sem sistema de ficheiros e sem rede — uma extensão em script vem do repositório de um desconhecido, por isso recebe o que declarou e mais nada.

| | |
|---|---|
| `storage.get(key)` / `storage.set(key, value)` | as suas definições, no seu directório. Só cadeias. Precisa de `storage.local` |
| `t(key)` | a sua cadeia na língua de quem lê; uma chave desconhecida volta como ela própria |
| `require(name)` | um dos seus ficheiros, resolvido só dentro do directório da sua extensão |
| `ctx.command` | o `id` da entrada de menu ou do comando que disparou |
| `ctx.selection` | o texto seleccionado, `""` quando nada está seleccionado |
| `ctx.document` | o documento inteiro |
| `ctx.answer` | o que quem lê escreveu da última vez que perguntou, senão nil/undefined |
| `ctx.view` | como quem lê está a ver o documento: `source`, `preview` ou `split` |
...
|---|---|
| `document.read` | ler o documento aberto e a selecção |
...
| `storage.local` | guardar o seu ficheiro de definições no seu directório |
...
por trás**: não existe barra de ferramentas, barra de estado nem paleta de
...
Os valores estão em `settings.json`, no directório da própria extensão, por isso nenhuma extensão consegue ler nem escrever as definições de outra. O que quem lê guarda chega a um script já em execução no comando seguinte, e não no próximo arranque.
...
- **Não corra se não foi o editor a arrancá-lo.** O editor gera uma senha em cada arranque e passa-a na variável de ambiente `MARKTEXT_PLUS_PLUGIN_TOKEN`; sem ela, `serve()` imprime uma linha a dizer o que é este programa e termina com 1. Uma senha feita para um arranque não pode ser escrita por quem não a recebeu, e viaja no ambiente e não em argv, que pode ser lido por tudo o que saiba executar `ps`.

  **O que isto faz e o que não faz.** Nenhum programa consegue impedir que um ficheiro no disco de quem lê seja executado — um duplo clique no seu executável arranca sempre um processo. O que a senha torna infalsificável é a sua resposta a «foi o editor que me arrancou?», de modo que o processo arranca, diz o que é, e termina, em vez de ficar sentado no stdin a parecer um programa encravado. Uma extensão que salta esta verificação é uma extensão que ninguém protegeu; é por isso que a verificação está no `serve()` do SDK e não apenas neste parágrafo. As extensões em script recebem isto de graça: um ficheiro `.lua` ou `.js` é interpretado pelo editor, e um duplo clique abre, quando muito, um editor de texto.
...
Uma extensão é um ficheiro na máquina de outra pessoa, que este editor lê. Parti-la não é uma falha de compilação que alguém aqui veja — é uma extensão que deixa de funcionar para quem a usa. Por isso:
...
- Nunca escreva uma chave de API no directório da extensão nem no manifesto.
- Para o stdout não vai nada que não sejam mensagens do protocolo.
- Delimite o trabalho você mesmo. Um script corre na própria thread do editor
