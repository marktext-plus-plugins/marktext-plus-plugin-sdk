...
`‡` — エディタはコンパイル済みプラグインを**まだ起動しません**。インストールはでき、
そのコマンドを実行すると *"has no script to run: its runtime is process"* と正直に伝えます。
以下のプロトコルは将来話す言葉であり、それを話すホスト側はエディタ内に実装済みでテストもあります。
ただ、コマンドをそこへ渡す箇所がまだありません。コンパイルに費やす一晩が
驚きではなく判断になるよう、ここに書いておきます。
...
`manifest.json` を読めなかったプラグインは一覧に出ません。そして**一覧にない**ことは**一度もインストールしていない**のと見分けがつかないので、たいていはもう一度インストールし、また何も出てきません。

エディタはそれを読み込めたものの下に別枠で並べ、**理由も書きます**：読めなかったキーと、そこに何があるはずだったか。その行には削除ボタンが付いています——読み込めないプラグインには、アンインストールするためのページがないからです。

理由はたいてい次のどれかです:

- `runtime` は `lua`、`js`、`process`、`data` のいずれか
- `lua` と `js` には `entrypoint`、`process` には `entrypoints`（OS ごとに 1 つのパス）が要ります
- 入口を `.dart` ファイルにはできません。エディタは Dart SDK を同梱しておらず、読者が持っている前提にもできません——コンパイルして、`runtime: "process"` で実行ファイルを配布してください
...
パネルから実行したコマンドは二度目に実行されることがあります。読者は最初から
すべてを説明し直すのではなく、答えを変えてほしいと頼むのです。エディタはその
ことを、スクリプトがすでに知っているフィールドで伝えます:

- `ctx.selection` は**あなた自身の前回の答え**です。追加の要求は文書ではなく、
  読者が見ている草稿に向けられているからです。
- `ctx.answer` は今回頼まれた内容 — あなたの `ask` の答えが届くのと同じ
  フィールドです。

つまり、選択範囲を「手を入れる部分」として読むコマンドは何も変える必要が
ありません。`ctx.selection` を無視するコマンドは文書全体を毎回書き直すことに
なり、それは読者が求めたものではありません。

この入力欄は、コマンドが最初に何かを尋ねた場合にのみ現れます。追加の要求は
そこに届くからです。何も尋ねないコマンドは一度だけ実行されます。`apply` で
置き換えられるのは常に**最初の答え**が指した箇所です。短い二稿目も、一稿目が
作られた段落を置き換えます。二稿目の元になった草稿は文書のどこにもありません。
...
| `{ pane = "<html>…", title = "…", as = "web" }` | そのペインにあなた自身の HTML ページを、システムの Web エンジンで描きます。`ui.webview` が必要 | stops |
| `{ notify = "…" }` | 読者に一行伝える | 終了 |
...
**画像。** `source` は三つのうちのどれかです：

- `data:` URI——その場でデコードされ、何もこの機械の外へは出ません；
- あなたのプラグインのディレクトリからの相対パス——絶対パスは拒否されます。読者のディスク上のファイルを読むのは `workspace.read` の仕事です；
- `http(s)` の URL——**エディタが取りに行きます**。あなたのプラグインではありません。だから読者のシステムプロキシに従い、ホスト名・ステータス・サイズ・所要時間があなたのプラグインのログに書かれます。`network.request` が要ります。

最後の一行は意図したものです。あなたはネットワークに出てよい。読者はどこへ行ったかを知ってよい。**あとから誰も確かめられない権限は、権限ではなく約束です。**
...
| `pane` 且 `as = "web"` | `ui.webview`。これは `ui.sidebar` と `network.request` を連れてきます |

`ask`、`show`、`diff` は何も要りません。コマンドを実行した読者に答えるのはプラグインの本分であり、
そこに権限を求めれば結局どのプラグインも宣言することになります——全員が持つ権限は、読者に何も伝えません。

**自分のページ。** `as = "web"` は、ペインのテキストを言葉ではなく HTML ページとして描きます。
エディタが設計しなかった画面に至る唯一の道であり、同時に高くつく道です。ほかに返せるものは
すべて**エディタに向けた記述**——名前のついたノード、あるいはテキスト——であり、
エディタ自身のウィジェットで描かれます。だからこそ起動時に何も要さず、
テーマを知らなくても読者のテーマに従います。

エンジンは OS のもの——Windows は WebView2、macOS は WKWebView——で、
**エディタに同梱したブラウザではありません**。ダウンロード容量とメモリはそのままで、
読者自身のプロキシ設定も誰の手配もなく効きます。そして**求められるまで作りません**：
この種のプラグインを一度も開かない読者は、エンジンの代価を払いません。

ページがどこへ行ったかはプラグインのログに書かれます。それが `ui.webview` の約束です。
**アドレスではなく HTML を送ってください**：描くものはあなたのもので、
サーバーに何か要りたいページは自分で要求できます。

**すべての機械にあるとは限りません。** エディタが使える Web エンジンのない Linux では、
あなたのプラグイン名を挙げた一文が出ます——空白のペインではありません。空白は
あなたのプラグインが壊れたように見えます。それらの読者を意図して除くのでなければ、
Web ページを唯一の使い道にしないでください。
...
◆分割表示で読んでいる文書は、この四つのうち**二つ**を占めます——ソースとプレビューがすでに一つの分割だからです。したがって分割表示の文書の横に入るペインは二つまでで、三つは入りません。三つ目のスロットを埋めても、読者が分割表示をやめるまで描かれません。

区切りはドラッグできます。ソースとプレビューの間のものと同じです。

スロット名 `right`、`bottom`、`corner` は、あとでそのペインを指し示す宛先でもあります——追記したり、中身を差し替えたりするための。ペイン一つはやはりペイン一つです: `corner` だけを埋めれば文書の隣にペインが一つ得られ、隅の一マスと空の二マスではありません。ただしどちら向きかは名前が決めます——`bottom` だけを埋めると文書の下に、ほかのスロットなら隣に置かれます。いま見ている段落の書き直しは、同じ行幅で下に置かれたほうが読みやすく、目が横ではなく下へ移るからです。読み手はあとからペインのタイトル行で入れ替えられます。エディタの知らないスロット名は推測されずに拒否されます: 頼んでいない場所に現れ、なぜかを知る手立てもないペインは、断られるより悪いのです。
...
| 入れ子の関数内で値のない `return` | `return nil` | 無視され、その後の行がそのまま走ります |

**最後の一つは一行では足りません。** `if not ok then return end` は Lua を書く誰もがガードを書くやり方ですが、入れ子の関数の中では何も守りません。飛ばすはずだった行がそのまま実行されます。検証は通り、空の場合も「処理」され、その処理は起きていません。

`while true do ... end` の中ではもっと悪くなります。ループに出口がなく、読者にはエディタが応答しなくなったように見えます。

直しは一語です——`return nil` と書いてください。普通の Lua なので、インタプリタが直った日にもそのまま動きます。

エディタ自身のテストがこれらを留めているので、インタプリタが置き換わればこの表は訂正されます。誤解を招いたまま残ることはありません。値のない `return` の挙動は、それが**壊れていると主張する**テストで留めてあります。そのテストが失敗する日が、この回避策を外せる日です。
...
**`toolbar` と `pages` が最後の二つなのは意図的です。まだ何も描画しません。**
ここで宣言したツールバーボタンは現れず、`pages` は読み込まれた後どこからも参照されません。
プラグイン自身の設定ページは下の `settings` から作られ、そちらは描画されます。
どちらも manifest の一部なので残してあり、機能が揃えば尊重されます——
上の `†` が付いた権限と同じです。その前の三つは今日描画されます。

`title` は翻訳キーでもかまいません。`location` はエディタが定めるスロットです——プラグインは名前のあるスロットに物を置くだけで、ピクセル座標を指定することも、自前のウィジェットをエディタに渡すこともできません。

`panels` は右サイドバーにアイコンを置きます。押すと引き出しが開き、同じ `id` のコマンドを実行した結果で埋まります。`ui.sidebar` が要り、`icon` も要ります——あの帯はアイコンの列であり、描くもののないアイコンは「押すと何かが開く空白」になるからです。パネルを出すプラグインが一つもなければ帯そのものが現れません。アイコンのないアイコン帯は、文書から取り上げただけの幅です。

パネルは自分の引き出しの中で尋ねます。`ask` を返したコマンドは、問い・指定した `choices`・前回の答えがすでに入った入力欄をそこに置き、返事も同じ引き出しに届きます。以前は浮かぶカードで尋ねて答えは引き出しに落ちていました——一つのやり取りが二か所に分かれていたのです。引き出しを閉じるのが読者の断り方です。メニューから始めたコマンドは今もカードで尋ねます。自分の場所を持たないからです。
...
**エディタはまだこれを起動しません** — 冒頭の `‡` を参照してください。
以下は将来話すプロトコルで、エディタ側の半分は実装済みです。コンパイル済みプラグインは
インストールでき、そのコマンドを実行すると実行するスクリプトがないと伝えます。
この節が述べるのはすべてプラグイン側です。
...
**なぜエディタが読み込むライブラリではなくプロセスなのか。** Lua と JS のプラグインはすでにエディタのプロセス内で、その同じスレッドで動いています——それが通常の姿で、それが成り立つのは解釈実行だからです：まずいスクリプトはエディタが捕まえられる例外を投げるので、間違いはクラッシュではなくメッセージで終わります。それが覆わないのは、戻ってこないスクリプトです——それは同じようにウィンドウを凍らせるので、上の規則はあなた自身に仕事の区切りをつけるよう求めています。

ネイティブコードは残りの境界も取り払います。スレッドはアドレス空間を共有するので、読み込まれた `.so` のどこかでのセグメンテーション違反、スタックオーバーフロー、`abort()` は、読者の保存していないドキュメントごとエディタを持っていき、報告する術もありません。アンロードは信頼できないので、「プラグインを無効にする」が実際には止めていません。別プロセスはそれらを取り戻します——落ちてもよく、タイムアウトで殺してもよく、エディタは生き残り、どのプラグインの仕業かを言えます。（Dart に関してはそもそも選択肢がありません：`dart compile` のサブコマンドは `exe`、`aot-snapshot`、`js`、`wasm` とスナップショット形式だけで、C から呼べる `.so` や `.dll` を作るものは**存在しません**。）
...
- 仕事の区切りは自分でつけること。スクリプトはエディタ自身のスレッドで動き、それを
  中断するものは何もありません——出口のないループは、誰かがプロセスを終了させるまで
  ウィンドウを凍らせます。タイムアウトがあるのはコンパイル型プラグインだけです。別
  プロセスであってはじめてタイムアウトが成り立つからです。
- パストラバーサルを含むプラグイン ZIP はインストール時に拒否されます。64 MB を
  超えるもの、エントリが 10000 を超えるもの、展開後に 256 MB を超えるものも同じです。
  三つのプラットフォーム分の実行ファイルなら収まります。リポジトリに入れたビルド
  ディレクトリは収まりません。
