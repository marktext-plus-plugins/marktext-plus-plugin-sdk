...
**ペイン。** エディタはもともとタブをソースとプレビューに分けています。`pane` はそれを開いたものです。ドキュメントが最初のマスを取り、あなたはさらに三つまで埋められます。

形はいくつ埋まっているかで決まり、どの段階でも左右対称です:

| 埋まった数 | 配置 |
|---|---|
| なし | ドキュメントがタブ全体を使う |
| 一つ | ドキュメントとあなたのペインが横並びで半分ずつ |
| 二つ | 上半分を左右に分け、二つめのペインが下半分を丸ごと取る |
| 三つ | 上下とも左右に分かれ、四つとも同じ大きさ |

区切り線はドラッグできます。ソースとプレビューの間のものと同じです。

スロット名 `right`、`bottom`、`corner` が言うのは**どのペインか**であって、どこに置かれるかではありません。あとからそのペインを指し示すための宛先です — 追記したり、中身を差し替えたりするための。`corner` だけを埋めれば、ドキュメントの隣にペインが一つ得られます。隅の一マスと空の二マスではありません。エディタの知らないスロット名は推測されずに拒否されます: 頼んでいない場所に現れ、なぜかを知る手立てもないペインは、断られるより悪いのです。
