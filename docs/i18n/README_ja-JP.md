...
| `{ pane = "…", title = "…", slot = "right"\|"bottom"\|"corner", apply = true, replaces = "…" }` | ドキュメント周りのペインの一つを埋める | 終了。ドキュメントには何も書かない |
| `{ notify = "…" }` | 読者に一行伝える | 終了 |
...
`ask`、`show`、`diff` は何も要りません。コマンドを実行した読者に答えるのはプラグインの本分であり、
そこに権限を求めれば結局どのプラグインも宣言することになります——全員が持つ権限は、読者に何も伝えません。

**ペイン。** エディタはもともとタブをソースとプレビューに分けています。`pane` はそれを開いたものです。マスは最大四つで、**分割表示の二つの半分がそのうちの二つです**——この仕組みはそこから作られたので、分割表示の文書はあなたが何かを埋める前にすでに二マスです。

形はマスがいくつあるかで決まり、どの段階でも左右対称です:

| マス | 配置 |
|---|---|
| 一 | 文書がタブ全体を使う |
| 二 | 横並びで半分ずつ |
| 三 | 片方の半分が分かれ、もう片方は丸ごと |
| 四 | 左上、右上、左下、右下 |

三マスで文書が分割表示でないとき、**どちらの半分を分けるかはあなたが選びます**: `right` を埋めると文書の隣にペインが来るので上が分かれ、もう一つのペインが下の行を丸ごと取ります。`bottom` と `corner` だけを埋めれば、文書が上の行を丸ごと持ち、二つのペインが下を分けます。読み手はあとからペインのタイトル行で入れ替えられます——どちらの半分が分かれているかは同じペインの見え方であって、あなたが決め続けるものではありません。

区切りはドラッグできます。ソースとプレビューの間のものと同じです。

スロット名 `right`、`bottom`、`corner` は、あとでそのペインを指し示す宛先でもあります——追記したり、中身を差し替えたりするための。ペイン一つはペイン一つで、何を名乗っていても同じです: `corner` だけを埋めれば文書の隣にペインが一つ得られ、隅の一マスと空の二マスではありません。エディタの知らないスロット名は推測されずに拒否されます: 頼んでいない場所に現れ、なぜかを知る手立てもないペインは、断られるより悪いのです。

**ペインは開かれたタブのものです。** タブを切り替えれば画面から消え、タブを閉じれば一緒に閉じ、戻ればまた現れます。

**書き戻しを申し出られます。** `apply` はペインに「適用」ボタンを付け、ペインが持っているものを文書に書きます。`replaces` は何を置き換えるかを言い、空なら文書全体です。モデルが返したものは、読み手が書いていた文章に入る前に読む価値がありますから、書き直しはまず示され、読み手が言ったときに書かれます。エディタの履歴を通るので、取り消し一回で戻ります。何を置き換えるかはあなたのコマンドが動いた時点で決まり、ボタンが押された時点ではありません: 途中で動いた選択範囲のせいで文章がどこか適当な場所に入ることはありません。`document.write` が要り、エディタはそれをボタンのところで確かめます。あなたのフラグを信用してではありません。

**始める前に、やっていると言うこと。** ペインの動作は `ai` より先に読まれるので、両方を返すのは「これを出してから、聞きに行け」という意味です。そして本文が空で背後に要求が控えているペインこそ、エディタが「作業中」として描くものです。最初のブロックが届くまでは本文の場所でそう言い、届いてからはタイトル行に移るので、進捗が届いたものの上に乗ることはありません。`ai` だけを返すと、モデルがかける数秒のあいだ画面は変わらず、何もしないメニュー項目のように読めます。

ペインを閉じるのが、読み手があなたを止める方法です。閉じられたペインへの追記は、ブロックごとに戻してくるのではなく拒否されます。
