...
`‡` — o editor **ainda não inicia** um plugin compilado. Ele instala, e executar um
de seus comandos diz exatamente isso:
*«has no script to run: its runtime is process»*. O protocolo abaixo é o que ele vai
falar, e o lado que o fala dentro do editor está escrito e testado; apenas nada
entrega um comando a ele. Escrito aqui para que a noite gasta compilando seja uma
...
Porque é distribuído com a sua extensão. `lib/marktext-plus.lua` não é uma dependência para a qual se aponta — é um arquivo que copia com o resto e que a partir daí é seu. Copiar `packages/lua` por inteiro dá-lhe uma extensão funcionando, API incluída; não há outro sítio de onde a ir buscar, nem número de versão a acompanhar.
...
Uma extensão cujo `manifest.json` não pode ser lido não aparece na lista, e o que falta na lista se parece exatamente com o que nunca foi instalado — então o passo seguinte costuma ser instalar de novo, e de novo nada aparece.

O editor a lista à parte, abaixo das que carregaram, **com o motivo**: a chave que não conseguiu ler e o que esperava encontrar ali. Essa linha tem seu próprio botão de exclusão, porque uma extensão que não carrega não tem página de onde ser desinstalada.

O motivo quase sempre é um destes:

- `runtime` precisa ser `lua`, `js`, `process` ou `data`
- `lua` e `js` exigem `entrypoint`; `process` exige `entrypoints`, um caminho por sistema operacional
- nenhum ponto de entrada pode ser um arquivo `.dart`. O editor não traz um SDK do Dart nem pode supor que o leitor tenha um — compile e distribua o executável com `runtime: "process"`
...
| `select` | `id` (obrigatório), `options`, `value` — um menu suspenso, quando são demais para chips |
| `checkbox` | `id` (obrigatório), `label`, `value`; o valor chega como `"true"` ou `"false"` |
| `markdown` | Markdown, desenhado pelo renderizador do próprio editor — sua resposta fica com a cara do documento de que trata |
| `image` | `source` (obrigatório), `height` — veja abaixo |
...
- uma URI `data:` — decodificada no lugar, nada sai da máquina;
- um caminho relativo à pasta do seu plugin — um absoluto é recusado, porque ler arquivos do disco de quem lê é tarefa do `workspace.read`;
- uma URL `http(s)` — **quem busca é o editor**, não o seu plugin: assim segue o proxy do sistema e fica registrado no log do seu plugin com o host, o status, o tamanho e o tempo. Precisa de `network.request`.

Essa última linha é proposital. Você pode ir à rede; quem lê pode saber aonde. **Uma permissão que ninguém consegue conferir depois é uma promessa, não uma permissão.**
...
navegador distribuído com o editor**. A baixa e a memória ficam onde
...
avariada. Não faça de uma página web a única forma de usar a sua extensão, a menos que queira deixar esses leitores de fora.
...
◆Um documento lido em visualização dividida ocupa duas dessas células, não uma — fonte e pré-visualização já são uma divisão. Ao lado de um documento dividido há espaço para dois painéis e não três: um terceiro slot não é desenhado até que o leitor saia da visualização dividida.

Os separadores arrastam-se, como o que há entre código-fonte e pré-visualização.

Os nomes `right`, `bottom`, `corner` são também a morada por onde você volta a um quadro mais tarde, para lhe acrescentar ou substituir o que ele tem. Um quadro continua sendo um quadro: preencher apenas `corner` dá a você um quadro ao lado do documento, e não um canto com duas células vazias à frente. O sentido, esse, quem decide é o nome — preencher apenas `bottom` põe o quadro embaixo do documento, e qualquer outra casa põe ao lado —, porque uma reescrita do parágrafo que você está lendo se lê melhor embaixo, com a mesma largura de linha, onde o olho desce em vez de atravessar. O leitor pode virá-lo pela barra de título do quadro. Um nome que o editor não conhece é recusado em vez de adivinhado: um quadro que aparece onde não o pediu, sem maneira de saber porquê, é pior do que uma recusa.
...
**O último merece mais do que uma linha.** `if not ok then return end` é como qualquer programador de Lua escreve uma guarda, e dentro de uma função aninhada ela não guarda nada: as linhas que deveria pular são executadas do mesmo jeito. Sua validação passa, o caso vazio fica "tratado", e o tratamento não acontece.

Dentro de `while true do ... end` é pior. O laço não tem saída, e o que o leitor vê é um editor que parou de responder.

A correção é uma palavra — escreva `return nil`. É Lua comum, e continuará funcionando no dia em que o interpretador for corrigido.

O conjunto de testes do próprio editor fixa estes pontos, por isso se o interpretador for substituído esta tabela será corrigida em vez de ficar induzindo em erro. O comportamento do `return` sem valor está fixado por um teste que afirma que ele está **quebrado**: o dia em que esse teste falhar será o dia em que o contorno pode sumir.
...
nunca mais olhado — a página de configurações do próprio plugin vem de `settings`
mais abaixo, e essa é desenhada. Os dois continuam no manifesto porque fazem parte
dele e serão honrados quando a capacidade chegar, como as permissões `†` acima.
...
Um painel pergunta na própria gaveta: um comando que devolve `ask` coloca ali a pergunta — o texto, as `choices` que você nomeou e uma caixa que já traz a resposta da última vez — e a resposta chega na mesma gaveta. Antes a pergunta aparecia no cartão flutuante enquanto a resposta caía na gaveta: uma só troca em dois lugares. Fechar a gaveta é como o leitor recusa. Um comando iniciado por um menu continua perguntando no cartão, pois não tem lugar próprio para isso.
...
**O editor ainda não inicia nenhum** — veja `‡` mais acima. O que segue é o
protocolo que ele vai falar, e a metade que vive no editor está escrita: um plugin
compilado instala, e executar um de seus comandos diz que não tem script para
rodar. Tudo nesta seção descreve o lado do plugin.

O executável é arrancado como processo filho e fala JSON-RPC 2.0, um objeto JSON por linha, em stdin/stdout. As respostas devolvem o `id` numérico. [`packages/dart/lib`](../../packages/dart/lib), neste repositório, implementa isso para extensões escritas em Dart e compiladas com `dart compile exe`.
...
- Um ZIP de extensão com uma entrada que sai da pasta é recusado na instalação;
  o mesmo vale para um acima de 64 MB, um com mais de 10000 entradas, ou um que
  descompacte para mais de 256 MB. Um executável para três plataformas cabe; uma
  pasta de build versionada não.
