...
`‡` —— 编辑器**还不会启动**编译型插件。它能安装，运行它的命令时会照实说：
*「has no script to run: its runtime is process」*。下面那套协议是它将来要讲的话，
而讲这套话的宿主在编辑器里已经写好并有测试；只是没有任何地方把命令派发给它。
写在这里，是为了让你花一晚上去编译它成为一个决定，而不是一个意外。
...
从面板运行的命令可能被运行第二次——读者要求把答案改一改，而不是从头把要求
再描述一遍。编辑器用你的脚本已经认识的字段来说这件事：

- `ctx.selection` 是**你自己上一次的答案**，因为追加的要求针对的是读者眼前
  这一稿，不是整篇文档。
- `ctx.answer` 是他这次的要求——就是你那个 `ask` 问题被回答的同一个字段。

所以，把选中的文字当作「要处理的那部分」的命令，什么都不用改。
而忽略 `ctx.selection` 的命令，会一次次从整篇文档重写，那不是读者要的。

只有当命令一开始就问过问题时才会出现这个输入框，因为追加的要求正是从那里
送进来的；从不提问的命令只运行一次。点「采用」时要替换的，始终是**第一个
答案**认定的那段原文——第二稿更短，替换的仍是第一稿所依据的那个段落，
而不是它自己的上一稿，那段文字根本不在文档里。
...
| `{ pane = "<html>…", title = "…", as = "web" }` | 在那个分栏里画出你自己的 HTML 页面，用系统自带的网页引擎。需要 `ui.webview` | stops |
| `{ notify = "…" }` | 告诉用户一句话 | 结束 |
...
- 相对于你插件目录的路径——绝对路径会被拒绝，读用户磁盘上的文件是 `workspace.read` 的事；
- `http(s)` 网址——**由编辑器去取**，不是你的插件，所以它跟着读者的系统代理走，并且带着主机名、状态码、大小和耗时写进你插件的日志。需要 `network.request`。

最后那条是有意的。你可以访问网络；读者可以查出你去了哪里。**事后无人可查的权限只是一个承诺，不是权限。**
...
| `pane` 且 `as = "web"` | `ui.webview`，它连带 `ui.sidebar` 与 `network.request` |

`ask`、`show`、`diff` 不需要任何权限。回答刚运行了命令的用户，本来就是插件该做的事；
为此要求声明权限，等于要求每个插件都声明它——而一个人人都持有的权限，对用户不传递任何信息。

**你自己的页面。** `as = "web"` 会把分栏里的文字当作一个 HTML 页面来画，而不是当作文字。
这是通往「编辑器没有设计过的界面」的唯一一条路，也是最贵的一条：你能返回的其他一切
都是**描述给编辑器听**的——具名节点，或者文字——由编辑器用自己的控件画出来，
所以那些在启动时不花一分钱，也不需要你知道读者用的是什么主题就能跟着它走。

引擎是操作系统的——Windows 用 WebView2、macOS 用 WKWebView——**不是随编辑器打包的浏览器**，
所以安装包和内存占用都留在原处，读者自己的代理设置也不需要谁来安排就生效。
而且**不开就不创建**：从不打开这类插件的读者，永远不会为一个引擎付代价。

页面访问了哪里会写进插件日志，这正是 `ui.webview` 承诺的事。
**发 HTML，不要发网址**：你画的东西是你自己的，页面想从服务器要什么可以自己去要。

**不是每台机器都有引擎。** 装不上网页引擎的 Linux 机器会看到一句话，点名是你的插件——
而不是一个空白分栏，那看起来像是你的插件坏了。除非你有意把这些读者排除在外，
否则不要让网页成为使用你插件的唯一方式。
...
◆在分屏里读的文档占这四格中的**两格**，不是一格——源码与预览本身就是一次分割。所以分屏文档旁边只放得下两个分栏，不是三个：填了第三个槽位，在读者离开分屏之前它不会被画出来。

分隔条可以拖动，和源码与预览之间那根一样。

槽位名 `right`、`bottom`、`corner` 也是你之后再找到某个分栏的地址，用来追加或替换它的内容。一个分栏仍然只是一个分栏：只填 `corner` 得到的是文档旁边的一个分栏，不是角落里的一格加两格空白。但方向由槽位名决定——只填 `bottom` 会放在文档下面，填别的则在旁边——因为改写你正在看的这一段，放在它下面才好读：行宽相同，眼睛往下扫而不是往旁边看。读者之后可以从分栏标题栏把它翻过来。编辑器不认识的槽位名会被拒绝而不是猜一个：一个出现在你没要的位置、又无从得知为什么的分栏，比一句拒绝更糟。
...
解释器是纯 Dart 实现的 Lua，这正是脚本插件不需要装任何东西的原因——而它并不完整。下面四条**全部静默失败**，代价就在这里：一个匹配不到东西的模式，看起来和"文档里本来就没东西"一模一样。

| 不要用 | 改用 | 因为 |
|---|---|---|
| `#someString` | `string.len(s)` | 抛 `length error`。`#` 对**表**是正常的，所以凭感觉分辨不出来 |
| `s:match("%S")`、`%s` | 逐字符比较：`s:sub(i, i) == " "` | 这些字符类匹配不到任何东西，于是每一行都"像空行" |
| `for l in s:gmatch("(.-)\n")` | `s:find("\n", pos, true)` 配 `s:sub` | 什么都不返回 |
| `s:gmatch("[^\n]*")` | 同上 | 空匹配不前进 |

| 嵌套函数里不带值的 `return` | `return nil` | 它被忽略，后面的语句照样执行 |

**最后这条值得单独说。** `if not ok then return end` 是每个 Lua 程序员写守卫的方式，而在嵌套函数里它什么也守不住：本该跳过的那几行照样跑。你的校验通过了，空值情况"处理"了，而处理并没有发生。

在 `while true do ... end` 里更糟：循环没有出口，读者看到的是编辑器不响应了。

改动只有一个词——写 `return nil`。它是普通的 Lua，解释器将来修好了也照样对。

编辑器自己的测试套件钉住了这几条，所以将来换掉解释器时，这张表会被纠正，而不是留着误导人。裸 `return` 那条是用一个**断言它坏掉**的测试钉住的：那条测试哪天失败了，就是可以拿掉这个绕法的那天。

JavaScript 那边是 QuickJS，没有值得单列的同类缺口。
...
**`toolbar` 与 `pages` 排在最后两位是有意的：现在还没有东西画它们。**
在这里声明的工具栏按钮不会出现，而 `pages` 被读进来之后就再没人看过一眼——
插件自己的设置页来自下面的 `settings`，那个是画出来的。两者仍然留在 manifest 里，
因为它们是 manifest 的一部分，能力到位时会被兑现，与上面打 `†` 的权限一样。
它们前面那三个今天是画出来的。

`title` 可以是翻译键。`location` 是编辑器定义的槽位——插件只能把东西放进有名字的槽位，不能指定像素坐标，也不能把自己的控件交给编辑器。

`panels` 在右侧边栏放一个图标；按下它会打开一个抽屉，内容由运行同 `id` 的命令填充。它需要 `ui.sidebar`，也需要一个 `icon`——那条栏就是一列图标，一个画不出东西的图标会变成一处「按了会打开什么」的空白。没有任何插件贡献面板时，那条栏根本不出现——一条没有图标的图标栏，是白白从文档那里拿走的宽度。

面板在自己的抽屉里提问：返回 `ask` 的命令把问题放在那里——问题本身、你给的 `choices`、以及一个已经填着上次答案的输入框——答复也回到同一个抽屉。以前问题弹在浮动卡片里而答案落在抽屉里，一次对话分在两个地方。关掉抽屉就是读者拒绝的方式。从菜单启动的命令仍然在卡片里问，因为它没有属于自己的地方。
...
**编辑器还不会启动它**——见顶部的 `‡`。下面是它将来要讲的协议，
而编辑器这一侧已经写好了一半：编译型插件能安装，运行它的命令会说自己没有脚本可跑。
这一节描述的全是插件那一侧。
...
**为什么是进程，而不是编辑器加载的库。** Lua 和 JS 插件本来就在编辑器进程里、同一个线程上跑——那是常规情况，之所以成立是因为它们是被解释执行的：脚本出错抛的是编辑器能接住的异常，一个失误的结局是一条消息而不是一次崩溃。它盖不住的是一个永不返回的脚本；那同样会冻住窗口，所以上面那条规则要你自己给工作划边界。

原生代码把剩下的边界也拿掉了。线程共享地址空间，所以一个 `.so` 里任何位置的段错误、栈溢出或 `abort()`，都会带走编辑器和用户未保存的文档，而且无从上报；卸载也不可靠，"禁用插件"其实并没有真的停掉它。独立进程把这些都拿回来了——它可以崩、可以超时被杀，编辑器活着并且说得清是哪个插件干的。（对 Dart 而言另外没有选择：`dart compile` 的子命令只有 `exe`、`aot-snapshot`、`js`、`wasm` 和几种快照格式，**不存在**产出 C 可调用的 `.so` 或 `.dll` 的子命令。）
...
- 工作的边界要你自己划。脚本跑在编辑器自己的线程上，没有任何东西会打断它：一个没有
  出口的循环会冻住窗口，直到有人杀掉进程。只有编译型插件有超时，因为只有它们是独立
  进程、才谈得上超时。
- 带有路径穿越条目的插件 ZIP 会在安装时被拒绝；超过 64 MB 的、条目多于 10000 个的、
  解压后超过 256 MB 的，同样会被拒绝。三个平台的可执行文件装得下；一个提交进仓库的
  构建目录装不下。
