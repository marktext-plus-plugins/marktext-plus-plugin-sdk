...
**分栏。** 编辑器本来就把一个标签页分成源码和预览；`pane` 就是把这件事开放出来。文档占第一格，你最多可以再填三格。
...
| 零 | 文档占满整个标签页 |
| 一 | 文档与你的分栏左右并排，各占一半 |
| 二 | 上半左右对分，你的第二个分栏占满整个下半 |
| 三 | 上下都左右对分，四格等大 |

分隔条可以拖动，和源码与预览之间的那一根一样。

槽位名 `right`、`bottom`、`corner` 说的是**哪一个分栏**，不是它落在哪里：它们是你事后再找到这个分栏的地址——往里追加内容，或者替换其中的内容。只填 `corner`，得到的是文档旁边的一个分栏，而不是角落里的一格加两格空白。编辑器不认识的槽位名会被拒绝而不是猜一个：一个出现在你没要的位置、又无从得知为什么的分栏，比一句拒绝更糟。
