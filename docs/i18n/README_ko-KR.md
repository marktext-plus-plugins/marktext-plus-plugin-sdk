...
`manifest.json`을 읽을 수 없는 플러그인은 목록에 나오지 않습니다. 그리고 **목록에 없는 것**은 **한 번도 설치하지 않은 것**과 똑같아 보이므로, 대개 다시 설치하게 되고 또다시 아무것도 나타나지 않습니다.

에디터는 그것을 불러온 플러그인들 아래에 따로 나열하고 **이유도 적습니다**: 읽지 못한 키와 그 자리에 무엇이 있어야 했는지. 그 줄에는 삭제 버튼이 따로 있습니다 — 불러올 수 없는 플러그인에는 제거할 수 있는 자기 페이지가 없기 때문입니다.

이유는 대개 다음 중 하나입니다:

- `runtime`은 `lua`, `js`, `process`, `data` 중 하나여야 합니다
- `lua`와 `js`에는 `entrypoint`가, `process`에는 운영체제마다 하나씩의 경로를 담은 `entrypoints`가 필요합니다
- 진입점은 `.dart` 파일일 수 없습니다. 에디터는 Dart SDK를 함께 배포하지 않고 사용자가 가지고 있다고 가정할 수도 없습니다 — 컴파일한 뒤 `runtime: "process"`로 실행 파일을 배포하세요
...
패널에서 실행한 명령은 두 번째로 실행될 수 있습니다. 독자가 전체를 다시
설명하는 대신 답을 고쳐 달라고 요청하는 것입니다. 에디터는 이를 스크립트가
이미 아는 필드로 전합니다:

- `ctx.selection` 은 **당신이 방금 낸 답**입니다. 후속 요청은 문서가 아니라
  독자가 보고 있는 초안을 향하기 때문입니다.
- `ctx.answer` 는 이번에 요청한 내용 — `ask` 의 답이 도착하는 그 필드입니다.

따라서 선택 영역을 "손볼 부분"으로 읽는 명령은 아무것도 바꿀 필요가 없습니다.
`ctx.selection` 을 무시하는 명령은 매번 문서 전체를 다시 쓰게 되며, 그것은
독자가 요청한 바가 아닙니다.

이 입력란은 명령이 처음에 무언가를 물었을 때만 제공됩니다. 후속 요청이 바로
그곳으로 도착하기 때문입니다. 아무것도 묻지 않는 명령은 한 번만 실행됩니다.
`apply` 가 바꾸는 대상은 언제나 **첫 답**이 가리킨 곳입니다. 더 짧은 두 번째
초안도 첫 초안이 만들어진 문단을 대체하며, 그 초안 자체는 문서 어디에도
없습니다.
...
| `{ pane = "<html>…", title = "…", as = "web" }` | 그 창에 당신의 HTML 페이지를 시스템 웹 엔진으로 그립니다. `ui.webview`가 필요합니다 | stops |
...
**그림.** `source` 는 셋 중 하나입니다:

- `data:` URI — 그 자리에서 디코딩되며, 아무것도 이 기계 밖으로 나가지 않습니다;
- 플러그인 디렉터리 기준 상대 경로 — 절대 경로는 거부됩니다. 독자의 디스크에 있는 파일을 읽는 것은 `workspace.read` 의 일입니다;
- `http(s)` 주소 — **에디터가 가져옵니다**. 플러그인이 아니라. 그래서 독자의 시스템 프록시를 따르고, 호스트·상태·크기·시간이 플러그인 로그에 적힙니다. `network.request` 가 필요합니다.

마지막 줄은 일부러 그렇게 했습니다. 네트워크에 나가도 됩니다. 독자는 어디에 갔는지 알 수 있어야 합니다. **나중에 아무도 확인할 수 없는 권한은 권한이 아니라 약속입니다.**
...
| `pane` 且 `as = "web"` | `ui.webview`, 이것은 `ui.sidebar`와 `network.request`를 함께 데려옵니다 |

`ask`, `show`, `diff`는 아무것도 필요하지 않습니다. 명령을 실행한 사람에게 답하는 것이
플러그인의 본분이고, 여기에 권한을 요구하면 결국 모든 플러그인이 그것을 선언하게 됩니다 —
모두가 가진 권한은 읽는 사람에게 아무것도 알려주지 않습니다.

**당신의 페이지.** `as = "web"`은 창의 글을 말이 아니라 HTML 페이지로 그립니다.
에디터가 설계하지 않은 화면으로 가는 유일한 길이고, 동시에 값비싼 길입니다. 돌려줄 수 있는
다른 모든 것은 **에디터에게 설명하는 것**입니다 — 이름 붙은 노드, 또는 글 — 그리고 에디터
자신의 위젯으로 그려집니다. 그래서 그것들은 시작할 때 아무것도 쓰지 않고, 테마를 몰라도
읽는 사람의 테마를 따릅니다.

엔진은 운영체제의 것입니다 — Windows는 WebView2, macOS는 WKWebView — **에디터와 함께
묶인 브라우저가 아닙니다**. 내려받는 크기와 메모리는 그대로이고, 읽는 사람의 프록시 설정도
누가 손대지 않아도 적용됩니다. 그리고 **요청받기 전에는 만들지 않습니다**: 이런 플러그인을
한 번도 열지 않는 사람은 엔진 값을 치르지 않습니다.

페이지가 어디로 갔는지는 플러그인 로그에 적힙니다. 그것이 `ui.webview`의 약속입니다.
**주소가 아니라 HTML을 보내세요**: 그리는 것은 당신의 것이고, 서버에서 뭔가 필요한
페이지는 스스로 요청할 수 있습니다.

**모든 기계에 있는 것은 아닙니다.** 에디터가 쓸 수 있는 웹 엔진이 없는 Linux에서는
당신의 플러그인 이름을 밝힌 한 줄이 나옵니다 — 빈 창이 아닙니다. 빈 창은 당신의 플러그인이
고장난 것처럼 보입니다. 그 사람들을 일부러 제외할 생각이 아니라면, 웹 페이지를 유일한
사용 방법으로 만들지 마세요.
...
슬롯 이름 `right`, `bottom`, `corner`는 나중에 그 창을 다시 가리키는 주소이기도 합니다 — 덧붙이거나 내용을 바꾸기 위한. 창 하나는 여전히 창 하나입니다: `corner`만 채우면 문서 옆의 창 하나를 얻지, 구석의 한 칸과 빈 두 칸이 아닙니다. 다만 어느 쪽인지는 이름이 정합니다 — `bottom`만 채우면 문서 아래에, 다른 슬롯이면 옆에 놓입니다. 지금 보고 있는 문단을 고쳐 쓴 것은 같은 줄 너비로 아래에 놓일 때 읽기 좋고, 눈이 옆이 아니라 아래로 내려가기 때문입니다. 읽는 사람은 나중에 창 제목줄에서 바꿀 수 있습니다. 에디터가 모르는 슬롯 이름은 짐작되지 않고 거절됩니다: 요청하지 않은 곳에 나타나고 그 이유를 알 길도 없는 창은 거절보다 나쁩니다.
...
| 중첩된 함수 안의 값 없는 `return` | `return nil` | 무시되고, 그 뒤 줄이 그대로 실행됩니다 |

**마지막 것은 한 줄로는 모자랍니다.** `if not ok then return end` 은 Lua를 쓰는 사람이라면 누구나 쓰는 가드 형태지만, 중첩된 함수 안에서는 아무것도 막지 못합니다. 건너뛰어야 할 줄이 그대로 실행됩니다. 검증은 통과하고, 빈 경우도 "처리"되지만, 그 처리는 일어나지 않습니다.

`while true do ... end` 안에서는 더 나쁩니다. 반복문에 출구가 없고, 읽는 사람에게는 에디터가 응답하지 않는 것으로 보입니다.

고치는 데는 한 단어면 됩니다 — `return nil` 이라고 쓰세요. 평범한 Lua이므로 인터프리터가 고쳐지는 날에도 그대로 동작합니다.

에디터 자신의 테스트가 이것들을 붙잡아 두므로, 인터프리터가 바뀌면 이 표는 고쳐집니다. 값 없는 `return` 의 동작은 그것이 **망가져 있다고 단언하는** 테스트로 붙잡아 두었습니다. 그 테스트가 실패하는 날이 이 우회를 없앨 수 있는 날입니다.
...
`panels`는 오른쪽 사이드바에 아이콘을 놓습니다. 누르면 서랍이 열리고, 같은 `id`의 명령을 실행한 결과로 채워집니다. `ui.sidebar`가 필요하고 `icon`도 필요합니다 — 그 줄은 아이콘의 열이며, 그릴 것이 없는 아이콘은 "누르면 뭔가 열리는 빈칸"이 되기 때문입니다. 패널을 내놓는 플러그인이 하나도 없으면 그 줄 자체가 나타나지 않습니다. 아이콘 없는 아이콘 줄은 문서에서 빼앗은 폭일 뿐입니다.

패널은 자기 서랍 안에서 묻습니다. `ask`를 돌려준 명령은 질문과, 지정한 `choices`와, 지난번 답이 이미 들어 있는 입력란을 거기에 두고, 답도 같은 서랍으로 돌아옵니다. 예전에는 떠 있는 카드에서 묻고 답은 서랍에 떨어졌습니다 — 한 번의 주고받음이 두 곳에 나뉘어 있었습니다. 서랍을 닫는 것이 읽는 사람이 거절하는 방법입니다. 메뉴에서 시작한 명령은 지금도 카드에서 묻습니다. 자기 자리가 없기 때문입니다.
...
**왜 에디터가 불러오는 라이브러리가 아니라 프로세스인가.** Lua와 JS 플러그인은 이미 에디터 프로세스 안에서, 같은 스레드에서 돕니다 — 그것이 보통의 모습이고, 그것이 성립하는 것은 해석되기 때문입니다: 잘못된 스크립트는 에디터가 잡을 수 있는 예외를 던지므로, 실수는 충돌이 아니라 메시지로 끝납니다. 그것이 덮지 못하는 것은 돌아오지 않는 스크립트입니다 — 그것은 똑같이 창을 얼리므로, 위의 수칙은 스스로 일에 끝을 두라고 합니다.

네이티브 코드는 나머지 경계마저 없앱니다. 스레드는 주소 공간을 공유하므로 불러온 `.so` 어디에서든 세그먼테이션 오류, 스택 넘침, `abort()`가 나면 읽는 사람의 저장하지 않은 문서와 함께 에디터를 가져가고, 보고할 방법도 없습니다. 언로드는 믿을 수 없어서 "플러그인 끄기"가 실제로는 멈추지 않습니다. 별도 프로세스는 그것들을 되찾아 줍니다 — 죽어도 되고, 시간 초과로 죽여도 되며, 에디터는 살아남아 어느 플러그인 탓인지 말할 수 있습니다. (Dart에 대해서는 애초에 고를 것이 없습니다: `dart compile`의 하위 명령은 `exe`, `aot-snapshot`, `js`, `wasm`과 스냅숏 형식뿐이고, C가 부를 수 있는 `.so`나 `.dll`을 만드는 것은 **없습니다**.)
...
- 일의 끝은 직접 지으세요. 스크립트는 에디터 자신의 스레드에서 돌고, 그것을 중단시키는
  것은 아무것도 없습니다. 빠져나갈 곳이 없는 루프는 누군가 프로세스를 죽일 때까지 창을
  얼립니다. 시간 제한이 있는 것은 컴파일된 플러그인뿐입니다. 별도 프로세스라야 시간
  제한을 걸 수 있기 때문입니다.
- 경로를 벗어나는 항목이 있는 플러그인 ZIP은 설치할 때 거부됩니다. 64 MB를 넘는 것,
  항목이 10000개를 넘는 것, 풀었을 때 256 MB를 넘는 것도 마찬가지입니다. 세 플랫폼치
  실행 파일은 들어갑니다. 저장소에 넣은 빌드 디렉터리는 들어가지 않습니다.
