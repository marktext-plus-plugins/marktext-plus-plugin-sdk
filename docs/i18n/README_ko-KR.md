...
**창.** 에디터는 원래 탭을 소스와 미리보기로 나눕니다. `pane`은 그것을 열어 준 것입니다. 문서가 첫 칸을 차지하고, 여러분은 최대 세 칸을 더 채울 수 있습니다.

모양은 몇 칸이 채워졌는지가 정하며, 어느 단계에서나 좌우 대칭입니다:

| 채운 수 | 배치 |
|---|---|
| 없음 | 문서가 탭 전체를 차지 |
| 하나 | 문서와 여러분의 창이 나란히, 절반씩 |
| 둘 | 위 절반을 좌우로 나누고, 두 번째 창이 아래 절반을 통째로 |
| 셋 | 위아래 모두 좌우로 나뉘어 네 칸이 같은 크기 |

구분선은 소스와 미리보기 사이의 그것처럼 끌 수 있습니다.

슬롯 이름 `right`, `bottom`, `corner`가 말하는 것은 **어느 창인가**이지 어디에 놓이는가가 아닙니다. 나중에 그 창을 다시 가리키기 위한 주소입니다 — 덧붙이거나, 담긴 것을 바꾸기 위한. `corner`만 채우면 문서 옆의 창 하나를 얻습니다. 구석의 한 칸과 빈 두 칸이 아닙니다. 에디터가 모르는 슬롯 이름은 짐작되지 않고 거절됩니다: 요청하지 않은 곳에 나타나고 그 이유를 알 길도 없는 창은, 거절보다 나쁩니다.
