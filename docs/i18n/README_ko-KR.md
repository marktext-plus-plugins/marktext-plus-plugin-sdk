...
`‡` — 편집기는 컴파일된 플러그인을 **아직 시작하지 않습니다**. 설치는 되고,
그 명령을 실행하면 *"has no script to run: its runtime is process"* 라고 그대로 알립니다.
아래 프로토콜은 앞으로 말하게 될 언어이고, 그것을 말하는 호스트는 편집기 안에
이미 작성되어 테스트까지 있습니다. 다만 명령을 그쪽으로 넘기는 곳이 아직 없습니다.
컴파일에 쓰는 하룻밤이 놀라움이 아니라 결정이 되도록 여기에 적어 둡니다.
...
◆분할 보기로 읽는 문서는 이 네 칸 가운데 **두 칸**을 차지합니다 — 원본과 미리 보기가 이미 하나의 분할이기 때문입니다. 그래서 분할된 문서 옆에는 창이 두 개까지만 들어가고 세 개는 들어가지 않습니다. 세 번째 슬롯을 채워도 독자가 분할 보기를 벗어날 때까지 그려지지 않습니다.
...
**`toolbar` 와 `pages` 가 마지막 두 개인 것은 의도된 것입니다. 아직 무엇도 그리지 않습니다.**
여기에 선언한 툴바 버튼은 나타나지 않고, `pages` 는 읽힌 뒤 다시 보이지 않습니다 —
플러그인 자신의 설정 페이지는 아래의 `settings` 에서 나오며 그쪽은 그려집니다.
둘 다 manifest 의 일부이므로 남겨 두었고, 기능이 준비되면 지켜집니다.
위의 `†` 권한과 같습니다. 그 앞의 세 개는 오늘 그려집니다.
...
**편집기는 아직 이것을 시작하지 않습니다** — 맨 위의 `‡` 를 보십시오.
아래는 앞으로 말하게 될 프로토콜이며, 편집기 쪽 절반은 이미 작성되어 있습니다.
컴파일된 플러그인은 설치되고, 그 명령을 실행하면 실행할 스크립트가 없다고 말합니다.
이 절이 설명하는 것은 모두 플러그인 쪽입니다.
